...
/**
 * Aplica o resultado do pagamento Mercado Pago no domínio (uma transação JPA por notificação),
 * após o payload já ter sido obtido da API do MP (fora desta transação).
 */
@Service
public class MercadoPagoWebhookPaymentApplier {

    private static final Logger log = LoggerFactory.getLogger(MercadoPagoWebhookPaymentApplier.class);
    ...
    public MercadoPagoWebhookPaymentApplier(PagamentoService pagamentoService,
    ...
    @Transactional
    public void aplicarWebhookPagamento(String mercadoPagoPaymentId, JsonNode payment) {
        ...
        if ("approved".equalsIgnoreCase(mpStatus)
            ...
            log.info(
                    "Mercado Pago webhook | idempotência | paymentId={} | ignorando reprocessamento (já APPROVED no banco)",
        ...
        log.warn(
                "external_reference não reconhecido (nem assinatura nem pedido): {} | payment={}",
                externalRef,
                mercadoPagoPaymentId
        );
    }
